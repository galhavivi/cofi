...
export default function reducer(forms = {}, action = {}) {
  switch (action.type) {
    ...
    case Steps.START_PROCESSING:
      {
        const newForms = cloneShallowModel(forms, action.formId);
        newForms[action.formId].model.processing = true;
        return newForms;
      }
    case Steps.END_PROCESSING:
      {
    ...
    case Steps.SET_FORM_INITIALIZED_DATA:
      {
        const newForms = cloneShallowModel(forms, action.formId);
        newForms[action.formId].model.initializedData = action.data || {};
        return newForms;
      }
    case Steps.SET_FORM_EVALUATION:
      {
        const newForms = cloneShallowModel(forms, action.formId);
    ...
    case Steps.SET_FIELD_VALUE:
      ...
      case Steps.SET_FIELD_EVALUATION:
      {
        const newForms = cloneShallowModel(forms, action.formId);
    ...
    case Steps.SET_FIELD_UI:
      ...
      case Steps.SET_FIELD_COMPONENT_STATE:
      {
        const newForms = cloneShallowModel(forms, action.formId);
        const field = cloneShallowField(newForms, action);
      ...
      case Steps.SET_FIELD_COMPONENT_PREV_STATE:
      {
        const newForms = cloneShallowModel(forms, action.formId);
        const field = cloneShallowField(newForms, action);
        field.component = cloneShallow(field.component);
      ...
      case Steps.SET_FIELD_COMPONENT_VALUE:
      {
        const newForms = cloneShallowModel(forms, action.formId);
        const field = cloneShallowField(newForms, action);
      ...
      case Steps.SET_FIELD_COMPONENT_VALUE_BATCH:
      {
        const newForms = cloneShallowModel(forms, action.formId);
        const { model } = newForms[action.formId];
