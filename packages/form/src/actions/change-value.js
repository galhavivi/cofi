...
export default function changeValue({ formId, fieldId, value }) {
  return (dispatch, getState) => new Promise((resolve) => {
    ...
    if (!verifyValidValue(fieldId, form, value, resolve)) return;

    value = evaluateValue(fieldId, model, value);

    // set view value to form
...
const verifyValidValue = (fieldId, form, value, resolve) => {
  if (isFunction(value) && !form.model.fields[fieldId].component) {
    const ERROR_PREFIX = 'changeValue -';
    const error = createError(ERROR_PREFIX, errors.CHANGE_VALUE_UPDATER_NOT_SUPPORTED, form, { fieldId }, []);
    log.error(error);
    resolve();
    return false;
  }
...
const setViewValueToStore = (formId, fieldId, value, dependencies = []) => (dispatch, getState) => {
  ...
  if (form.model.fields[fieldId].component) {
    ...
    dispatch(setFieldComponentValue(formId, fieldId, value), action);
  }
};
...
function trackDependencies(formId, fieldId, value, form, dependencies) {
  ...
  if (isMaxLoops) {
    throwError(prefix, errors.MAX_CIRCULAR_DEPENDENCIES_LOOPS, form, { dependencies },
      [
