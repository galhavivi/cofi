...
CofiError.prototype.toString = function toString() {
  return `Cofi error - "${this.code}": ${this.message}. More info: ${this.reference}.`;
};

export function createError(prefix, error, form, data, args = [], subError) {
  const message = error.message.apply(null, args);
  const referenceHash = error.code.replace(/_/g, '-').toLowerCase();
  const reference = `${COFI_DOCS_ERROR_CODES}#${referenceHash}`;
  return new CofiError(error.code, `${prefix} ${message}`, form, data, reference, subError);
}

export function throwError(prefix, error, form, data, args) {
  throw createError(prefix, error, form, data, args);
}
