...
describe('actions / creator', () => {
  it('startAction - return correct action', () => {
    ...
    const actionData = { formId: 'simple' };
    const action = startAction(formId, actionId, actionType, actionData);

    expect(action).toEqual({ type: 'START_ACTION', formId, actionId, actionType, actionData });
  });
  ...
  it('endAction - return correct action', () => {
    ...
    const actionType = 'INIT';
    const actionData = { formId: 'simple' };
    const actionMetadata = { start: 50, end: 73 };
    const action = endAction(formId, actionId, actionType, actionData, actionMetadata);

    expect(action).toEqual({ type: 'END_ACTION', formId, actionId, actionType, actionData, actionMetadata });
  });
