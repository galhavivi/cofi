...
describe('Form', () => {
  ...
  describe('create form', () => {
    it('create few forms without id', async () => {
      ...
      let result = await form.init(simpleForm.model, simpleForm.resources);
      expect(result).toBeTruthy();
      expect(form.id).toEqual('form-1');

      form = new Form();
      delete simpleForm.model.id;
      result = await form.init(simpleForm.model, simpleForm.resources);
      expect(result).toBeTruthy();
    ...
    it('init failed on verify form', async () => {
      log.error = jest.fn();
      let publicForm;

      const form = new Form();
      const result = await form.init({}, simpleForm.resources, undefined, x => { publicForm = x; });
      ...
      // verify that no actions occurred
      expect(publicForm).toEqual(undefined);
    });
    ...
    it('init with toDto', async () => {
      ...
      let publicForm;
      const form = new Form();
      const result = await form.init(simpleForm.model, simpleForm.resources, undefined, x => { publicForm = x; });
      expect(result).toBeTruthy();
    ...
    it('init persistent form with toDto and reset', async () => {
      ...
      let publicForm;
      let form = new Form();
      let result = await form.init(simpleForm.model, simpleForm.resources, undefined, x => { publicForm = x; });
      expect(result).toBeTruthy();
      ...
      form = new Form();
      result = await form.init(publicForm.model, simpleForm.resources, undefined, x => { publicForm = x; });
      expect(result).toBeTruthy();
      expect(form.data).toEqual(expectedData);
      expect(publicForm.model.data).toEqual(expectedData);
      expect(publicForm.model.initializedData).toEqual(expectedDataAfterFirstInit);
  ...
  describe('destroy form', () => {
    it('form should throw an error when try to call action after destroy', async () => {
      ...
      expect(form.destroyed).toEqual(false);
      await form.destroy();
      expect(form.destroyed).toEqual(true);
